...
#include "custom_armbot_hardware/dof6bot_hardware_interface.hpp"
...
namespace custom_armbot_hardware
{
    ...
    //---------------------------------------------------------------------------------------
    hardware_interface::CallbackReturn DOF6BOTSystem::on_init(const hardware_interface::HardwareInfo & info)
    {
    ...
    // METHOD read: 
    //---------------------------------------------------------------------------------------
    hardware_interface::return_type DOF6BOTSystem::read(const rclcpp::Time &, const rclcpp::Duration & period)
    {
        for (auto i = 0; i < int(joint_velocities_command_.size()); i++)
        {
            joint_velocities_[i] = joint_velocities_command_[i];
            joint_position_[i] += joint_velocities_command_[i] * period.seconds();
        }

        for (auto i = 0; i < int(joint_position_command_.size()); i++)
        {
...
PLUGINLIB_EXPORT_CLASS(custom_armbot_hardware::DOF6BOTSystem, hardware_interface::SystemInterface)
